    ...
    // compound shapes by populating them with a pre-transformed windmill
{
    ...
    this.populate(this, points_transform); // Otherwise, a new star immediately populates its own arrays with windmill points,
    this.init_buffers(); // Then sends its arrays to the graphics card into new buffers
}
inherit(star, shape);
star.prototype.populate = function (recipient, points_transform) // The meat of the star starts here:
{
    var offset = recipient.vertices.length; var index_offset = recipient.indices.length; // Recipient's previous size
    //vertices from back and front inner pentagon
    for (var z = -1; z < 2; z += 2) {
        recipient.vertices.push(vec3(0, 0, 0.5 * z));
        for (var i = 0, i_max = 5; i < i_max; i++) {
            var rotate = rotation(i * 360 / i_max, 0, 0, 1);
            var newPoint = mult_vec(rotate, vec4(0, 0.5, 0, 1));
            recipient.vertices.push(vec3(newPoint[0], newPoint[1], 0.25 * z));
        }
    }
    //vertices from outer pentagon
    for (var i = 0, i_max = 5; i < i_max; i++) {
        var rotate = rotation(i * 360 / i_max, 0, 0, 1);
        var newPoint = mult_vec(rotate, vec4(0, -1.5, 0, 1));
        recipient.vertices.push(vec3(newPoint[0], newPoint[1], 0));
    }

    //front
    recipient.indices.push(0, 1, 2, 0, 2, 3, 0, 3, 4, 0, 4, 5, 0, 5, 1);
    recipient.indices.push(1, 2, 15, 2, 3, 16, 3, 4, 12, 4, 5, 13, 5, 1, 14);
    //back
    recipient.indices.push(6, 7, 8, 6, 8, 9, 6, 9, 10, 6, 10, 11, 6, 11, 7);
    recipient.indices.push(7, 8, 15, 8, 9, 16, 9, 10, 12, 10, 11, 13, 11, 7, 14);
    //side
    recipient.indices.push(1, 7, 14, 5, 11, 14, 5, 11, 13, 4, 10, 13, 4, 10, 12, 3, 9, 12, 3, 9, 16, 2, 8, 16, 2, 8, 15, 1, 7, 15);
    ...
    // compound shapes by populating them with a pre-transformed windmill
{
    shape.call(this);
    if (!arguments.length) return; // Pass no arguments if you just want to make an empty dummy object that inherits everything, for populating other shapes
    this.populate(this, points_transform); // Otherwise, a new octahedron immediately populates its own arrays with windmill points,
    this.init_buffers(); // Then sends its arrays to the graphics card into new buffers
}
inherit(octahedron, shape);
octahedron.prototype.populate = function (recipient, points_transform) // The meat of the star starts here:
{
    var offset = recipient.vertices.length; var index_offset = recipient.indices.length; // Recipient's previous size

    //vertices
    recipient.vertices.push(vec3(0, 4, 0), vec3(0, 0, -1), vec3(1, 0, 0));
    recipient.vertices.push(vec3(0, 4, 0), vec3(1, 0, 0), vec3(0, 0, 1));
    recipient.vertices.push(vec3(0, 4, 0), vec3(0, 0, 1), vec3(-1, 0, 0));
    recipient.vertices.push(vec3(0, 4, 0), vec3(-1, 0, 0), vec3(0, 0, -1));

    recipient.vertices.push(vec3(0, -1, 0), vec3(0, 0, -1), vec3(1, 0, 0));
    recipient.vertices.push(vec3(0, -1, 0), vec3(1, 0, 0), vec3(0, 0, 1));
    recipient.vertices.push(vec3(0, -1, 0), vec3(0, 0, 1), vec3(-1, 0, 0));
    recipient.vertices.push(vec3(0, -1, 0), vec3(-1, 0, 0), vec3(0, 0, -1));

    //texture
    recipient.texture_coords.push(vec2(0, 0), vec2(1, 0), vec2(0, 1), vec2(0, 0), vec2(1, 0), vec2(0, 1), vec2(0, 0), vec2(1, 0), vec2(0, 1),
        vec2(0, 0), vec2(1, 0), vec2(0, 1), vec2(0, 0), vec2(1, 0), vec2(0, 1), vec2(0, 0), vec2(1, 0), vec2(0, 1), vec2(0, 0), vec2(1, 0), vec2(0, 1), vec2(0, 0), vec2(1, 0), vec2(0, 1));
    //indices
